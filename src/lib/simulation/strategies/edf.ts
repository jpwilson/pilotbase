...
function getSortedStudentsByDeadline(state: SimState): SimStudent[] {
  return [...state.students]
    .filter((s) => s.lessonQueue.length > 0)
    .sort((a, b) => {
      const aDeadline = a.lessonQueue[0].deadlineOffsetDays;
      const bDeadline = b.lessonQueue[0].deadlineOffsetDays;
...
export const edfStrategy: SimStrategy = {
  id: "edf",
  name: "Earliest Deadline First",
  description:
    "Prioritizes students whose next lesson has the nearest deadline. Students with no deadline are scheduled last.",
  keyMetric: "Minimizes deadline violations",

  onCancellation(cancelledBooking, state, _rng) {
    const sorted = getSortedStudentsByDeadline(state);
